...
export default function ScheduledOrderTable({
    ...
    const [orderTypeFilter, setOrderTypeFilter] = useState<Selection>("all");
    ...
    const filteredItems = useMemo(() => {
        ...
        if (
            orderTypeFilter !== "all" &&
            Array.from(orderTypeFilter).length !== orderTypeOptions.length
        ) {
            filteredOrders = filteredOrders.filter((order) =>
                Array.from(orderTypeFilter).includes(
    ...
    }, [orders, filterValue, orderFilter, orderTypeFilter, hasSearchFilter]);
    ...
    const renderCell = useCallback(
        (order: ScheduledOrderProps, columnKey: Key) => {
                    ...
                    <div className="flex items-end gap-3">
                        ...
                        <Dropdown>
                            ...
                            <DropdownMenu
                                ...
                                closeOnSelect={false}
                                disallowEmptySelection
                                onSelectionChange={setOrderTypeFilter}
                                selectedKeys={orderTypeFilter}
                                selectionMode="multiple"
                            >
                                {orderTypeOptions.map((column) => (
                                    <DropdownItem
                                        className="capitalize"
                                        key={column.uid}
                                    >
