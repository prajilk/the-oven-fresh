...
const EditPaymentDialog = ({
  ...
  const handleSubmit = () => {
    ...
    const promise = async () => {
      const result = await editPaymentAction({...valueState, orderId, orderType});
      setLoading(false);
      if (result.success) {
        return result;
      }
  ...
  function calculateTax() {
    ...
    const tax =
      ((subtotal + dFee) * Number(process.env.NEXT_PUBLIC_TAX_AMOUNT || 0)) / 100;
    return tax;
  }
  ...
  function managerDeliveryFeeChange() {
    ...
    if (valueState.tax > 0) {
      ...
      const pending = Number((afterTotal - advancePaid - discount).toFixed(2));
      setValueState((prev) => ({
        ...prev,
  ...
  function manageTaxChange() {
    ...
    const pending = Number((total - valueState.advancePaid - valueState.discount).toFixed(2));
  ...
  function manageDiscountChange() {
    ...
    const advancePaid = Number(valueState.advancePaid) || 0;
    const pending = Number((total - advancePaid - discount).toFixed(2));
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="scrollbar-thin max-h-[calc(100vh-50px)] overflow-y-scroll sm:max-w-[60%]">
        ...
        <div className="mt-5 grid grid-cols-4 bg-gray-50 [&>div]:border [&>div]:border-gray-200 [&>div]:p-3 [&_input]:bg-white">
          <PaymentBlock
          ...
          <div className="flex flex-col gap-2">
            ...
            <Select
              ...
              onValueChange={(value) => setValueState((prev) => ({...prev, paymentMethod: value}))}
              name="paymentMethod"
            >
            ...
            <Select
              ...
              onValueChange={(value) =>
                setValueState((prev) => ({
                  ...
                  pendingBalance: value === 'true' ? 0 : valueState.pendingBalance,
                }))
              }
        ...
        <DialogFooter>
          ...
          <ShadButton disabled={loading} type="button" onClick={handleSubmit}>
            Save changes
          </ShadButton>
        </DialogFooter>
      </DialogContent>
